...
class GetContentByTitlePaging @Inject constructor(val repository: PagingRepository): UseCase<List<ContentItemPage>, GetContentByTitlePaging.Params>() {
    class PrimaryParam(val title: String, val sortBy: RatingEnum = RatingEnum.Kinopoisk)
    ...
    override suspend fun run(params: Params): Either<Failure, List<ContentItemPage>> {
        return repository.getContentByTitle(params)
    }
}